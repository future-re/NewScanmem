...
#include "memseek/cli/commands/scan.hpp"
...
auto Application::run() -> int {
    ...
    if (const auto console = dynamic_cast<ui::ConsoleUI*>(m_ui.get()))
        console->setCompletionCallback([](const std::string_view prefix) {
            return getCommandCompletions(prefix);
        });
    if (m_config.initialCommands &&
        (executeCommandString(*m_config.initialCommands) || m_config.batchMode))
        return 0;
    if (m_config.batchMode) return 0;
    return REPL{m_ui, buildPrompt(), [this] { return buildPrompt(); }}.run();
}
auto Application::executeCommandString(const std::string& commands) -> bool {
    std::size_t start = 0;
    while (start <= commands.size()) {
        const auto end = commands.find(';', start);
        const auto command = commands.substr(start, end - start);
        if (!command.empty()) {
            const auto result = REPL::executeLine(command);
            if (result && result->shouldExit) return true;
        }
...
auto Application::getCommandCompletions(const std::string_view prefix)
    -> std::vector<std::string> {
    std::vector<std::string> candidates;
    for (const auto command : CommandRegistry::instance().getAllCommands()) {
        const auto name = std::string(command->getName());
        if (name.starts_with(prefix)) candidates.push_back(name);
        for (const auto alias : command->getAliases())
            if (std::string_view(alias).starts_with(prefix))
...
void Application::registerCommands() {
    ...
    registry.registerCommand(std::make_unique<commands::PidCommand>(m_session));
...
auto Application::buildPrompt() const -> std::string {
    return m_session.pid > 0 ? std::to_string(m_session.pid) + "> " : "> ";
}
}  // namespace cli
