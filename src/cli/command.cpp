...
auto Command::validateArgs(const std::vector<std::string>&) const
...
auto CommandRegistry::findCommand(const std::string_view name) -> Command* {
    auto key = std::string(name);
    if (const auto alias = m_aliases.find(key); alias != m_aliases.end())
        key = alias->second;
    const auto found = m_commands.find(key);
    return found == m_commands.end() ? nullptr : found->second.get();
}
auto CommandRegistry::getAllCommands() const -> std::vector<Command*> {
...
auto parseCommandLine(const std::string_view line)
