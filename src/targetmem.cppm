...
export {
     ...
     *
     * 字段:
     *  - maxNeededBytes: 为了预分配或限制使用的最大字节数（语义由调用方定义）。
    ...
    class MatchesAndOldValuesArray {
       public:
         ...
         *
         * 返回值:
         *  - 成功: std::pair<swath_ptr, index>，其中 swath_ptr 指向包含该匹配的
         *    MatchesAndOldValuesSwath，index 是 swath.data 中匹配元素的下标。
         ...
         *
         * 参数:
         *  - start, end: 要删除的地址区间（半开区间，包含 start 不包含 end）。
         *  - numMatches: 输出参数，删除过程中遇到的匹配（matchInfo !=
         * EMPTY）个数。
         *
         * 逻辑:
         *  - 遍历每个 swath 的 data（按字节存储的 OldValueAndMatchInfo 向量）。
         ...
         * 内，则将该元素视为要删除；同时如果它是一个匹配项则增加 numMatches。
