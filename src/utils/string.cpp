...
auto StringUtils::trim(std::string_view str) -> std::string_view { return trimLeft(trimRight(str)); }
...
auto StringUtils::trimRight(std::string_view str) -> std::string_view {
  const auto it = std::ranges::find_if_not(
      str | std::views::reverse, [](const unsigned char c) { return std::isspace(c) != 0; });
  return it == str.rend() ? std::string_view{}
                           : str.substr(0, static_cast<std::size_t>(it.base() - str.begin()));
}

auto StringUtils::split(std::string_view str, const char delimiter)
...
auto StringUtils::startsWith(const std::string_view str, const std::string_view prefix) -> bool {
  return str.starts_with(prefix);
}

auto StringUtils::endsWith(const std::string_view str, const std::string_view suffix) -> bool {
  return str.ends_with(suffix);
}

auto StringUtils::contains(const std::string_view str, const std::string_view substring) -> bool {
  return str.contains(substring);
}

auto StringUtils::join(const std::vector<std::string_view>& strings, const std::string_view delimiter)
    -> std::string {
