...
auto formatValueByType(const std::vector<std::uint8_t>& bytes,
                       ...
                       const bool bigEndian) -> std::string {
    if (!type || bytes.empty()) {
        const auto result = hexBytes(bytes);
        return result.empty() ? "0x00" : result;
    }
    const auto readValue = [&bytes, bigEndian]<typename T>() {
        T value{};
    ...
    switch (*type) {
        ...
        case ScanDataType::INTEGER_16:
            return std::format("{}",
                               readValue.template operator()<std::int16_t>());
        case ScanDataType::INTEGER_32:
            return std::format("{}",
                               readValue.template operator()<std::int32_t>());
        case ScanDataType::INTEGER_64:
            return std::format("{}",
                               readValue.template operator()<std::int64_t>());
        case ScanDataType::FLOAT_32:
            return std::format("{:.6g}",
                               readValue.template operator()<float>());
        case ScanDataType::FLOAT_64:
            return std::format("{:.15g}",
                               readValue.template operator()<double>());
