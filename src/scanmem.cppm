...
public:
    [[nodiscard]] static std::expected<config, std::string> parse(int argc, char* argv[]) {
        config result;
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (arg.starts_with("-p=") || arg.starts_with("--pid=")) {
                ...
                if (auto pid = parse_pid(pid_str)) {
                    ...
                    return std::unexpected{std::format("Invalid PID: {}", pid_str)};
                }
                continue;
            }
            
            if (arg.starts_with("-c=") || arg.starts_with("--command=")) {
                result.initial_commands = std::string{arg.substr(arg.find('=') + 1)};
                continue;
            }
...
public:
    ...
    execute_commands(const std::string& commands) {
        result res;
...
private:
    [[nodiscard]] static std::vector<std::string> split_commands(std::string_view commands) {
    ...
    [[nodiscard]] static result execute_single_command(std::string_view command) {
        // Placeholder for actual command execution
        // In a real implementation, this would integrate with the actual scanmem core
...
public:
    ...
    [[nodiscard]] std::expected<int, error> run() {
        ...
        if (cfg_.initial_commands) {
            auto result = command_processor::execute_commands(*cfg_.initial_commands);
            if (!result) {
                if (cfg_.exit_on_error) {
                    return 1;
                }
                show_message::error("Command execution failed: {}", result.error());
            }
        }
        
        if (cfg_.target_pid == 0) {
            show_message::user("Enter the pid of the process to search using the \"pid\" command.");
            show_message::user("Enter \"help\" for other commands.");
        } else {
            show_message::user("Please enter current value, or \"help\" for other commands.");
        }
...
private:
    ...
    [[nodiscard]] std::expected<int, error> run_interactive_loop() {
        ...
        while (true) {
            ...
            if (!result.success) {
                show_message::error("Command failed: {}", result.error_message.value_or("Unknown error"));
            }
        }
...
run_scanmem(const config& cfg) {
    ...
    try {
        ...
        if (result) {
            ...
            return std::unexpected{std::format("Session error: {}", result.error().message)};
        }
    } catch (const std::exception& e) {
