...
struct OldValueAndMatchInfo {
    std::uint8_t oldByte{};
    MatchFlags matchInfo{MatchFlags::EMPTY};
...
class MatchesAndOldValuesSwath {
   public:
    ...
    void markRangeByAddress(void* address, std::size_t length,
                            MatchFlags flags);
    [[nodiscard]] auto toPrintableString(
        std::size_t index, std::size_t length) const -> std::string;
    [[nodiscard]] auto toByteArrayText(std::size_t index,
                                       std::size_t length) const -> std::string;
};
