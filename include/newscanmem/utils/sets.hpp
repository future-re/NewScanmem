...
struct Set {
  ...
  static auto cmp(const std::size_t& left, const std::size_t& right) -> int {
    ...
    return result == std::strong_ordering::less ? -1 : result == std::strong_ordering::greater ? 1 : 0;
  }
};

[[nodiscard]] auto parseUintSet(std::string_view text, Set& set, std::size_t max_size) -> bool;
