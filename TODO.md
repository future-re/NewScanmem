...
- [ ] 统一类型视图接口 (ValueView)（优先级：高）
  - 实现 `ValueView<T>` 或类似适配器，支持 `value.as<int>()` / `value.set<float>(1.0)`。
  - 消除代码中手动 `memcpy` 和字节序处理的样板代码。
  - 验收：`scan/numeric` 和 `ui/user_input` 迁移使用新接口，并通过测试。

- [ ] 内存写入功能（优先级：高）
  - 实现 `process_write` 逻辑（`ptrace` 或 `/proc/mem` 写入）。
  - 添加 `write` 命令到 CLI。
  - 验收：可通过 CLI 修改目标进程内存值。
