...
TEST(ParseUintSetTest, ValidInput) {
    Set set;
    EXPECT_TRUE(parseUintSet("1,2,3", set, 10));
    EXPECT_EQ(set.size(), 3);
    EXPECT_EQ(set.buf, std::vector<size_t>({1, 2, 3}));

    EXPECT_TRUE(parseUintSet("0x1,0x2,0x3", set, 10));
    EXPECT_EQ(set.size(), 3);
    EXPECT_EQ(set.buf, std::vector<size_t>({1, 2, 3}));

    EXPECT_TRUE(parseUintSet("1..3", set, 10));
    EXPECT_EQ(set.size(), 3);
    EXPECT_EQ(set.buf, std::vector<size_t>({1, 2, 3}));
}
...
TEST(ParseUintSetTest, InvertedInput) {
    Set set;
    EXPECT_TRUE(parseUintSet("!1,2,3", set, 5));
...
TEST(ParseUintSetTest, InvalidInput) {
    Set set;
    EXPECT_FALSE(parseUintSet("1..10", set, 5));  // 超出范围
    EXPECT_FALSE(parseUintSet("abc", set, 10));   // 非法字符
    EXPECT_FALSE(parseUintSet("1..", set, 10));   // 不完整范围
}

TEST(ParseUintSetTest, EmptyInput) {
    Set set;
    EXPECT_FALSE(parseUintSet("", set, 10));  // 空输入
}
