...
TEST_F(ProcessCheckerTest, CheckCurrentProcess) {
    ...
    ProcessState state = ProcessChecker::check_process(current_pid);
...
TEST_F(ProcessCheckerTest, CheckDeadProcess) {
    ...
    ProcessState state = ProcessChecker::check_process(invalid_pid);
...
TEST_F(ProcessCheckerTest, CheckZeroPid) {
    ...
    ProcessState state = ProcessChecker::check_process(zero_pid);
...
TEST_F(ProcessCheckerTest, IsProcessDeadCurrent) {
    ...
    bool is_dead = ProcessChecker::is_process_dead(current_pid);
...
TEST_F(ProcessCheckerTest, IsProcessDeadInvalid) {
    ...
    bool is_dead = ProcessChecker::is_process_dead(invalid_pid);
