...
auto main(int argc, char** argv) -> int {
    ...
    constexpr std::uint64_t kExpectedValue = 1122334455667788ULL;
    constexpr std::uint64_t kModifiedValue =
        9999999999999999ULL;  // 期望被修改成的值
    ...
    while (gRunning.load() &&
        ...
        std::uint64_t currentValue = kMarker;

        // 检查值是否被修改为期望值
        if (currentValue == kModifiedValue) {
            std::cout << "SUCCESS: Value modified from 0x" << std::hex
                      << kExpectedValue << " to 0x" << currentValue << std::dec
                      << std::endl;
            valueModified = true;
            break;
        }

        std::cout << std::hex << "MARKER: 0x" << currentValue << std::dec
                  << std::endl;
